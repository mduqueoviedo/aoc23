...
for (let i = 0; i <= seeds.length - 2; i += 2) {
  seedMap.push({ initial: seeds[i], range: seeds[i + 1] });
}
...
seedMap.forEach((seedLine) => {
  for (let i = seedLine.initial; i < seedLine.initial + seedLine.range; i++) {
    let finalLocation = applyMappingToNumber(i, seedToSoil);
